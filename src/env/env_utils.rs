...
pub struct AppEnv {
    ...
    pub app_file_path_without_suffix: PathBuf,
    pub app_dir: PathBuf,
    pub app_file_name: String,
    pub app_file_name_without_suffix: String,
}
...
pub fn init_env() -> Result<(), EnvError> {
    ...
    // 获取当前执行文件路径(不带后缀)
    let mut app_file_path_without_suffix = app_file_path.clone();
    app_file_path_without_suffix.pop();
    ...
    // 获取当前执行文件名(不带后缀)
    let app_file_name_without_suffix = app_file_path
        .file_stem()
        .ok_or(EnvError::GetAppFileName())?
        .to_string_lossy()
        .to_string();
